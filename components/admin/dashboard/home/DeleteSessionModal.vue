    ...
    <div>
        <Teleport to="body">
            <CommonModal ref="modal">
                ...
                <table class="bg-white table-auto text-sm text-center mx-auto mb-2" v-if="sessionInfo">
                        ...
                        <tr>
                            <td class="px-6 py-2">
                                <div class="grid grid-cols-2 w-5/6 mx-auto gap-5 mb-5 ">
                                    ...
                                    <p class=" font-bold text-left">{{ sessionInfo.professional.first_name }} {{
                    sessionInfo.professional.last_name }} - {{ sessionInfo.professional.title }}</p>
                                </div>
                                <div class="grid grid-cols-2 w-5/6 mx-auto gap-5 mb-5 ">
                                    <p class="font-light text-right  ">Fecha</p>
                                    <p class=" font-bold text-left">{{ sessionInfo.date }}</p>
                                </div>
                                <div class="grid grid-cols-2 w-5/6 mx-auto gap-5 mb-5 ">
                                ...
                                <div class="grid grid-cols-2 w-5/6 mx-auto gap-5 mb-5 ">
                                    ...
                                    <p class=" font-bold text-left">{{ sessionInfo.modality }}</p>
                                </div>
                                <div class="grid grid-cols-2 w-5/6 mx-auto gap-5 mb-5 ">
                                    <p class="font-light text-right  ">Formato</p>
                                    <p class=" font-bold text-left">{{ sessionInfo.format }}</p>
                                </div>
                                <div class="grid grid-cols-2 w-5/6 mx-auto gap-5 mb-5 "
                                    ...
                                    <div>
                                        ...
                                        <p v-for="student in sessionInfo.students" class=" font-bold text-left">{{
                    student.email }}</p>
                                    </div>
                                </div>
                            </td>
                ...
                <div class="flex justify-center items-center p-5">
                    <CommonButton @click="deleteSession" bg-color="tertiary" class="px-4 py-2 rounded-md mr-2">
                        Eliminar sesión
                    </CommonButton>
...
const deleteSession = async () => {
    ...
    try {
        const response = await $fetch<APIResponse>(`${runtimeConfig.public.apiBase}/admin/session/${props.sessionInfo.session_id}`, {
            method: 'DELETE',
            credentials: 'include',
