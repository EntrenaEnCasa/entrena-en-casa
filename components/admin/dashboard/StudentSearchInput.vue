<template>
    <div class="relative">
        <div
            class="border bg-white text-gray-800 text-sm rounded-md px-5 py-3.5 w-full box-border"
            ...
            <div
                class="border rounded-md absolute top-[110%] left-0 right-0 bg-white shadow-lg p-3 z-40"
...
const fetchResults = async () => {
    if (searchTerm.value) {
        ...
        results.value = [];
        try {
            const response = await $fetch(`${runtimeConfig.public.apiBase}/admin/students/search`, {
                method: "POST",
                credentials: "include",
                body: {
            ...
            toast.error("Ocurrió un error al buscar los estudiantes");
        } finally {
